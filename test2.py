...
# 載入總索引（圖文配對紀錄）
if os.path.exists("dataset_index.json"):
    with open("dataset_index.json", "r", encoding="utf-8") as f:
...
def save_all():
    ...
    with open("dataset_index.json", "w", encoding="utf-8") as f:
...
while True:
    try:
        ...
        if img_path:
            ...
            filename = f"{counter:06d}_{text.replace(' ', '_')}{img_ext}"
